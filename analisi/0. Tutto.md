...
- **Finito**: se la successione è anche limitata. A questo punto:
  ...
  - Se è monotona decrescente e limitata inferiormente, il suolimite è l'estremo inferiore della successione
...
$$
\huge{q :=\lim_{k \to \infty}{\frac{a_k + 1}{ak}}}
$$
Allora la serie $\sum_{k=0}^\infty a_k$:
- Converge se $q < 1$
- Diverge se $q > 1$
...
$$
E vale che $|s - s_n| \lt a_{n+1}$ per ogni $n \in \mathbb{N}$ 
...
Se $f \in G[a,b]$ allora esistono $m = min \ f$ e $M = max \ f$ e in oltre:
- f è limitata
- $f(m) \le f(x) \le f(M)$ per ogni $x \in [a,b]$ 
- per ogni $f \in [m, M]$ esiste $x \in [a,b]$ tale che $f(x) = y$
...
Sia $f: (a,b) \in \mathbb{R}$ derivabile, e sia $x_0 \in (a,b)$ un punto critico in $f$ (possiamo trovarli tramite il teorema di fermal, cioè quando la derivata è 0), allora $x_0$ è:
...
Allora supponendo che $f$ sia $n$ volte derivabile, cerchiamo un polinomio $T_n$ che fino all'n-esimo termine ha il valore in $x_0$ in comune con tutte le derivate, cioè che:
...
Il resto di Peano ci dice che la velocità del resto tende a 0 per $x \to x_0$, mentr eil resto di lagrange ci dice quanto è grande il resto 
...
- $f$ è **iniettiva** $\Leftrightarrow$ ogni retta orizzontale attraverso un punto $y \in Y$ interseca $G(f)$ al più una volta.

- $f$ è **suriettiva** $\Leftrightarrow$ ogni retta orizzontale attraverso un punto $y \in Y$ interseca $G(f)$ almeno una volta.
- $f$ è **biettiva** $\Leftrightarrow$ ogni retta orizzontale attraverso un punto $y \in Y$ interseca $G(f)$ un’unica volta.
...
$$
Cioè che per ogni avvicinamento all'origine, risulta che il limite deve convergeree allo stesso valore $l$.
...
- $f$ è derivabile in $(x_0,y_0)$ rispetto a $x$ se esiste il limite:
$$
\lim_{h \to 0} \frac{f(x_0 + h, y_0) - f(x_0,y_0)}{h}
$$
- $f$ è derivabile in $(x_0,y_0)$ rispetto a $y$ se esiste il limite:
$$
...
Come abbiamo fatto nel caso degli integrali ad una variabile, vogliamo approssimare il volume tramite la differenza delle somme soperiori e inferiori. Ma in questo caso, non abbiamo una singola retta, ma un rettangolo.
...
$$
2. **x-semplice** se esistono due funzioni continue $h_1, h_2: [a,b] \to \mathbb R$ tali che:
$$
X = \left\{(x,y) \in \mathbb R^2 : x\in [a,b], \ \ h_1(x) \le y \le h_2(x)   \right\}
$$
