...
- **Divergente** se per $n \to \infty$, a successione tende a $\pm \infty$ 
...
Esempio: Trova un punto di Lagrange per $f:[0,2] -> \mathbb R, f(x) = x^3 - x$ 
Allora: 
...
Questa regola ci aiuta a trovare i limiti di certe forme indeterminate, cioè quando accade che durante la sostituzione abbiamo $\dfrac{0}{0}$ oppure $\dfrac{\pm \infty}{\pm \infty}$.
...
$$
\int_a^b \frac{1}{x} = ln(x) |^{e^2}_2 = ln(e^2) - ln(2) = 2
$$
