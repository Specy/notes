...
Dalla relazione di Mayer ([[04-il-primo-principio-della-termodinamica]]) sappiamo $C_P = C_V + R$. Definiamo il rapporto tra i due calori molari, l'**indice adiabatico**:
...
In una trasformazione **isoterma** la temperatura è costante. Poiché per un gas perfetto $U = U(T)$ (vedi [[04-il-primo-principio-della-termodinamica]]), si ha $\Delta U = 0$, quindi dal primo principio:
...
> $n = 5\ \text{mol}$ a $T = 300\ \text{K}$, da $P_A = 2\times 10^6\ \text{Pa}$, si espande **contro la pressione atmosferica costante** $P_0 \approx 10^5\ \text{Pa}$ (trasformazione irreversibile). Qui il lavoro si calcola con la pressione esterna (vedi [[03-equazione-di-stato-dei-gas-perfetti-e-trasformazioni]]):
